...
## This script creates a new directory of 'smaller' images
## from an existing directory of images. It also creates a
## mosaic file that groups all the smaller images together.
## 
...
##    to be loaded into the reduced images
## 
...
## ToDO Need to verify if this does the rotation correctly. ASimmonsSteffen 10.10.2022
                            elif FL_DIR == 'EW':
...
def key_func(k):
 ...
 # ToDO: explore other methods of reducing the file size more. Currently the size is being reduced using image parameters that are Format and Encoding or Pixel Depth.
 # We can also (if we choose) reduce image bands if we need only visible bands (RGB) and remove other bands like PIR....
